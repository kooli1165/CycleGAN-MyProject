...
from lib_add.augment import AugmentPipe
from datasets import ImageDataset

# fid v1.0 108.06  lr=0.0002
# my
# v0.1  输出3 mask 在up sample 阶段 分离
# v0.2  输出1维mask 进一步降低D lr
# v0.3  输出1维mask output layer 分离  不crop resize 崩了
#         仍旧crop resize

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--save_dir', type=str, default='./output/v1.0.1_dataset1.0')
    ...
    parser.add_argument('--lr_D', type=float, default=0.0004, help='initial learning rate')
    ...
    parser.add_argument('--ada_kimg', type=float, default=2)
    ...
    # ADA
    augment_pipe = AugmentPipe(opt.ada_start_p, opt.ada_target, opt.ada_interval, opt.ada_kimg, opt.ada_fixed).train()
