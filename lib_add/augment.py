...
from lib_add import SmoothUpsample, SmoothDownsample, identity
...
bgc = dict(xflip=1, rotate90=1, xint=1,
           scale=1, rotate=1, aniso=1, xfrac=1,
           brightness=1, contrast=1, lumaflip=1, hue=1, saturation=1)

class AugmentPipe(torch.nn.Module):
    def __init__(self, start_p, target, interval, kimg, fixed=False):
        super().__init__()
        ...
        self.xfrac = 1.  # Probability multiplier for fractional translation.
        ...
        self.lumaflip = 1  # Probability multiplier for luma flip.
    ...
    def heuristic_update(self, batch_size):
        ...
        self.p.copy_(self.p.min(torch.tensor(self.ada_target, device=self.p.device).float()))
...
if __name__ == "__main__":
    ...
    # image = read_image("../data/000011.jpg").unsqueeze(0).expand(4, -1, -1, -1).cuda() / 127.5 - 1
    # augment_pipe = AugmentPipe(0.5, 0.6, 4, 16, 1).cuda()
